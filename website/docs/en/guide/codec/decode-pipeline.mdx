...
The `decode` of a FieldCodec can be a plain function. `decurl/decode` provides helpers that write common parsing steps as shorter, more explicit, and easier-to-review pipelines.
...
```ts
import { elementOf } from 'decurl/decode';
...
```ts
import { where } from 'decurl/decode';
...
```ts
import { elementOf, mapItems, pipe, unique } from 'decurl/decode';
