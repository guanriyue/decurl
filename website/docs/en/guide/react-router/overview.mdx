---
description: "@decurl/react-router connects Decurl Search Fields to React Router and exposes URL search state as hooks."
---

# React Router Integration

`@decurl/react-router` connects Decurl Search Fields to React Router. Its goal is to let URL search be read and updated like ordinary React state, while keeping the URL as the final source of truth.
...
This style fits applications that only need one default search store.

Main-entry hooks do not require you to manually create a Provider. `useSearchValue` and `useSearchValues` use the default search store and automatically wire the current React Router runtime.
...
As long as the component is inside a React Router environment, it can use the main-entry hooks directly.
...
React Router hooks do not redefine URL fields. Search Fields are defined with `@decurl/react-router/codec` and then reused by hooks:
