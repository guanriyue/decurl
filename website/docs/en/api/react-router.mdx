...
The default `@guanriyue/decurl` entry provides ready-to-use React Router hooks. They wire the current React Router runtime inside the hook, so you do not need to place a Provider manually.
...
If you need to bind a store, isolate multiple React Router runtimes, or reduce direct dependency on React Router `useLocation`, see [@guanriyue/decurl/configured](./configured).
...
The default hooks wire the current React Router runtime directly. Some route updates may cause components to re-render with React Router. If you need to bind a store or observe render-count optimization, see [@guanriyue/decurl/configured](./configured) and [Bound Store](/guide/react-router/configured-store).
