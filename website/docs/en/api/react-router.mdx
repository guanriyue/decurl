---
description: 'API reference for the main-entry hooks from @decurl/react-router.'
---

# @decurl/react-router

The main `@decurl/react-router` entry provides ready-to-use React Router hooks. They wire the current React Router runtime inside the hook, so you do not need to place a Provider manually.

```tsx
import { useSearchValue, useSearchValues } from '@decurl/react-router';
```

The main-entry hooks use the default singleton search store. If an application has only one React Router runtime, this is usually enough.

These hooks must run inside a React Router context, such as `BrowserRouter`, `RouterProvider`, or another React Router runtime.

If you need to bind a store, isolate multiple React Router runtimes, or reduce direct dependency on React Router `useLocation`, see [@decurl/react-router/configured](./configured).
...
The main-entry hooks wire the current React Router runtime directly. Some route updates may cause components to re-render with React Router. If you need to bind a store or observe render-count optimization, see [@decurl/react-router/configured](./configured) and [Bound Store](/guide/react-router/configured-store).
