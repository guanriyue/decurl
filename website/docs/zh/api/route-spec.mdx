...
非 nullish path param 会先通过 `String(value)` 转换，再交给 React Router `generatePath`：
...
`search` 直接复用 Search Fields。生成 href 时，RouteSpec 把 search input 交给相同的 codec encode 能力，不重新定义 search 编码规则。
...
RouteSpec 不负责 search decode。页面可以直接复用它保存的 Search Fields：
