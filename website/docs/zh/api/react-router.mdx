---
description: '@guanriyue/decurl 默认 hooks 入口的 API reference。'
---

# @guanriyue/decurl

`@guanriyue/decurl` 默认入口提供开箱即用的 React Router hooks。它们会在 hook 内部接线当前 React Router runtime，不需要手动放置 Provider。
...
如果需要绑定 store、多 React Router runtime 隔离，或者减少组件对 React Router `useLocation` 的直接依赖，见 [@guanriyue/decurl/configured](./configured)。
...
默认 hooks 会直接接线当前 React Router runtime。某些路由更新可能让组件跟随 React Router 重新渲染；如果你需要绑定 store 或观察渲染次数优化，可以阅读 [@guanriyue/decurl/configured](./configured) 和 [绑定 Store](/zh/guide/react-router/configured-store)。
