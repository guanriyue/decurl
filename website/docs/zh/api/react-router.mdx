...
如果需要显式提供 store、配置 store 行为，或者减少组件对 React Router `useLocation` 的直接依赖，见 [@guanriyue/decurl/provided](./provided)。
...
默认 hooks 会在 hook 内部读取 React Router `location`。某些路由更新可能让组件跟随 React Router 重新渲染；如果你需要把 React Router 能力获取集中到独立组件，或观察相关渲染边界，可以阅读 [@guanriyue/decurl/provided](./provided) 和 [Provided Runtime](/zh/guide/react-router/provided-runtime)。
