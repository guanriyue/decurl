...
FieldCodec 的 `decode` 可以是普通函数。`@decurl/react-router/decode` 提供的是一组 helper，用来把常见解析步骤写成更短、更显式、更容易 review 的 pipeline。
...
这些转换对普通 JavaScript 是合法的，但在 URL schema 中通常不够可审查。Decode pipeline 倾向于先声明哪些字符串形状可以进入业务层。
...
- [Schema 与 codec](./schema-codec)：多个 field 如何组成一份 search object。
- [URLSearchParams](./url-search-params)：schema 如何 decode 和 patch encode URLSearchParams。
