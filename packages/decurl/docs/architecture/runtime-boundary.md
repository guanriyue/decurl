...
provided runtime connector 可以使用可订阅 runtime，例如基于 React Router router instance 的 runtime。
...
基于 router instance 的 runtime 不进入默认入口，但 provided 入口支持显式传入 router instance。
...
`router.subscribe` 在 React Router 类型中可能被标记为 private。router instance runtime 可以作为 provided 高级能力支持，但不应成为默认入口的使用方式。
