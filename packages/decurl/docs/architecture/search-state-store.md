...
它用于记录 entry 的原始上下文，也可以用于调试和更严格的失效判断。
...
pathname 是更高优先级的边界。只有当 incoming location 的 pathname 与当前 confirmed pathname 相同时，才会用 `search` 匹配 `inflightFlushes`。
...
普通 `addEntry` 不会每次全量 replay。它会将新 entry 增量 apply 到当前 `optimisticLocation`，以避免连续 add 时产生 O(N²) 计算。

entry remove、replace 或更复杂的 base 修正可以使用 replay 作为 dirty 重算路径。
...
Store 不关心 entry 来自 `useSearchValues`、`useSearchValue`，还是其他写入形态。
...
如果之后用户回到 previous 页面，URL 不会恢复这些未持久化的 optimistic entries。
...
- 保留 previous pathname 的 pending entries 会造成时序反转。
- 在离开 previous pathname 后继续序列化 previous search，可能制造额外导航或错误 history。
- pending 到 flush 的窗口通常很短，真实用户触发该边界的概率很低。

如果业务需要跨页面恢复 search state，应在业务侧显式保存状态，或降低 flush delay。
...
hook unmount 只取消订阅，不取消已经提交的 entry。
...
public setter 支持 navigate options。
...
如果 runtime navigate 失败、被 blocker 阻止，或没有收到对应 location confirmation，store 不暴露错误处理 API。

在未确认前，store 仍以 optimistic state 为准。

如果收到无法匹配 `inflightFlushes` 的 external location change，则按外部变化处理：
