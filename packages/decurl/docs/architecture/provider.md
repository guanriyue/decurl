...
使用 configured provided hooks 时，需要同时渲染 `SearchProvider` 和
...
这些能力由 configured subpath 中的 `SearchRuntimeConnector` 显式完成。
...
所有公开 hook 都应从当前 React context 读取 store。configured subpath 的
