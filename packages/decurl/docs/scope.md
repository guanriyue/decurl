...
`@guanriyue/decurl` 负责将 Decurl 的 URLSearchParams codec 能力接入 React 和 React Router。
...
- 使用 React Router `generatePath` 和 codec 生成类型安全 href。
- 保持 Provider、多 runtime 和高级能力所需的内部边界。

## Codec 边界

Codec/schema 约束由 `@guanriyue/decurl/codec` 与 `@guanriyue/decurl/decode` 提供。
...
这些能力不属于 `@guanriyue/decurl` 的默认 API 边界。
