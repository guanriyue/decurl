...
`encodeFieldValue(codec, value)` 负责执行 field 的值转换：

- 如果 value 是 `null | undefined`，返回 `undefined`。
- 如果 field 提供自定义 `encode`，使用自定义 `encode`。
- 如果没有自定义 `encode`，single value 默认使用 `String(value)`。
- 如果没有自定义 `encode`，multi value 默认逐项 `String(item)`，并过滤数组中的 `null | undefined`。
- 如果自定义 `encode` 返回 `null | undefined`，归一化为 `undefined`。

`encodeField(searchParams, codec, key, value, options)` 负责字段级 URLSearchParams 写入。它会返回新的 URLSearchParams，不修改传入对象。

字段级写入规则：

- key 必须由调用方明确传入。
- 如果 key 是数组，写入时第一个 key 是 canonical key，其余 key 是 alias。
- 如果 value 是 `null | undefined`，只删除候选 key，不写入新值。
- 如果 `encodeFieldValue` 返回 `undefined`，只删除候选 key，不写入新值。
- 如果 value 与 `defaultValue` 相等，且没有传入 `preserveDefault: true`，只删除候选 key，不写入新值。
- single field 使用 `set` 写入 canonical key，并删除 legacy alias。若 canonical key 已经存在，应保留 canonical key 的原始顺序位置。
- multi field 先删除所有候选 key，再使用 `append` 按顺序写入 canonical key。
