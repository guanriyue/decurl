...
`encodeField(codec, value, searchParams, key, options)` 负责字段级 URLSearchParams 写入。它会返回新的 URLSearchParams，不修改传入对象。

字段级写入规则：

- key 必须由调用方明确传入。
- 如果 key 是数组，写入时第一个 key 是 canonical key，其余 key 是 alias。
- 只要字段被写入，就会先删除所有候选 key。
...
- single field 使用 `set` 写入 canonical key。
- multi field 使用 `append` 按顺序写入 canonical key。
