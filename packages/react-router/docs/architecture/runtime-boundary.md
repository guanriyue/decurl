...
本文档用于记录 React Router runtime 与后续可替换 runtime 的边界。
...
P0 默认 hook 可以通过 React Router hooks 感知 location 变化，不要求 runtime 自身必须可订阅。

未来 Provider 高级形态可以使用可订阅 runtime，例如基于 React Router router instance 的 runtime。
...
无 Provider 时，`useSearchState` 使用默认 React Router runtime。
...
它不改变 search state，不应触发 subscribers，也不应导致 `useSearchState` 重新 render。
...
`hash` 和 route `state` 不纳入 P0 search state。
...
Provider runtime 是后续高级形态，不进入 P0。
...
`router.subscribe` 在 React Router 类型中可能被标记为 private。Provider runtime 可以作为高级能力支持，但不应成为 P0 默认使用方式。
...
Runtime P0 不实现基于 `window.navigation` 的版本。

该能力属于后续高级形态，且不能改变默认 React Router runtime 行为。
